...
val rowMapper: (ResultSet, Int) -> TaskApi.Out.Task = { rs, _ ->
    TaskApi.Out.Task(
        rs.getLong("id"),
        rs.getString("text"),
...
fun createTask(jdbcTemplate: JdbcTemplate, task: TaskApi.In.NewTask): Long? {
    ...
    val rs = jdbcTemplate.query(sql, task.text, Status.TODO.statusCode) { rs,_ -> rs.getLong("id") }
